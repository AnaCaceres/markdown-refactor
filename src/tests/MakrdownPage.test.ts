...
it.each([
  ...
  [
    "irrelevant [visible text link](url)",
    "irrelevant visible text link [^anchor1]\n\n[^anchor1]: url",
  ],
  [
    "[visible text link](url) irrelevant",
    "visible text link [^anchor1] irrelevant\n\n[^anchor1]: url",
  ],
  [
    "irrelevant [visible text link](url) irrelevant",
    "irrelevant visible text link [^anchor1] irrelevant\n\n[^anchor1]: url",
  ],
  [
    "[visible text link](url)[another visible text](url)",
    "visible text link [^anchor1]another visible text [^anchor2]\n\n[^anchor1]: url\n\n[^anchor2]: url",
  ],
  [
    "[visible text link](url), [another visible text](url)",
    "visible text link [^anchor1], another visible text [^anchor2]\n\n[^anchor1]: url\n\n[^anchor2]: url",
  ],
])("replace anchors by foot notes", (input, expected) => {
  const markdownWithFootNotes = new MarkdownPage(
    input
...
describe("findAnchorsAtPage", () => {
  it("should create a dictionary with an anchor at text", () => {
    ...
    const anchors = new MarkdownPage(inputContent).findAnchorsAtPage(
      inputContent
    );
  ...
  it("returns anchors without duplications given a text with same anchor", () => {
    ...
    const dictionary = new MarkdownPage(inputContent).findAnchorsAtPage(
      inputContent
    );
    const expected = [new Anchor("url", "visible text link")];
  ...
  it("returns anchors given a text with more characters after the last anchor", function () {
    const inputContent =
      "[visible text link](url), [other visible text link](other url) some text";
    const dictionary = new MarkdownPage(inputContent).findAnchorsAtPage(
      inputContent
    );
    const expected = [
...
describe("add footnotes", () => {
  it("should do nothing given no inputs", () => {
    ...
    expect(
      new MarkdownPage(inputContent).addFootNotes(
        inputContent,
        anchorsDictionary
      )
    ).toEqual("");
  ...
  it("should do nothing given a text without anchors", () => {
    ...
    ).toEqual(inputContent);
  });
  it("should add to footnotes anchor text from anchor dictionary", () => {
    expect(
      new MarkdownPage("[^anchor1]").addFootNotes("irr text [^anchor1]", {
